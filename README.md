...
1. **Análise Inicial dos Dados**: Importei a base (`clientes.csv`) com 100.000 registros e 25 colunas, entendendo os tipos de dados e distribuições.
...
3. **Divisão dos Dados**: Separei 70% dos dados para treino e 30% para teste.

4. **Treinamento dos Modelos**: Treinei dois algoritmos distintos:
   - **Random Forest (Árvore de Decisão em Conjunto)**
   - **KNN (K-Vizinhos Mais Próximos)**

5. **Avaliação e Escolha**: Comparei a acurácia de ambos.
   - ✅ **Random Forest: 82.27%** (Modelo Vencedor)
   - ❌ **KNN: 73.72%**

6. **Previsão para Novos Clientes**: Utilizei o modelo treinado (Random Forest) para prever o score de uma base de novos clientes (`novos_clientes.csv`), aplicando o mesmo pré-processamento.
